...
func getChatTestDB(t *testing.T) *pgxpool.Pool {
	t.Helper()
	...
	// Clean up chats table before each test
	_, err = pool.Exec(context.Background(), "DELETE FROM chat")
	if err != nil {
		t.Fatalf("Failed to clean chats table: %v", err)
	}

	return pool
}

func TestPushChats_Integration(t *testing.T) {
	if testing.Short() {
...
func TestPullChats_Integration(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	tests := []struct {
		...
		},
		{
			...
			wantStatus: 200,
			checkResp: func(t *testing.T, resp pullResp) {
				// Should get the second chat (or none if cursor is past it)
				if len(resp.Upserts) > 1 {
...
func TestPushPullRoundTrip_Chats_Integration(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	if len(pullResp.Upserts) != 1 {
		t.Fatalf("Expected 1 chat, got %d", len(pullResp.Upserts))
	}
...
func TestSoftDelete_Chats_Integration(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	if pullResp.Deletes[0]["uid"] != "c1d9b7dc-a1b2-4c3d-9e8f-7a6b5c4d3e2f" {
		t.Errorf("Wrong chat in deletes: %v", pullResp.Deletes[0])
	}
}
